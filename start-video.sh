...
gst-client pipeline_create atak interpipesrc listen-to=h264src block=true is-live=true allow-renegotiation=true stream-sync=compensate-ts ! nvv4l2decoder ! queue ! omxh264enc ${encoder_bitrate}=${ATAK_BITRATE} iframeinterval=60 ! h264parse ! mpegtsmux ! rtpmp2tpay ! udpsink host=${ATAK_HOST} port=${ATAK_PORT} ${extra_atak}
