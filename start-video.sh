...
gst-client pipeline_create edge interpipesrc listen-to=h264src block=false is-live=true allow-renegotiation=true stream-sync=compensate-ts ! nvv4l2decoder disable-dpb=true ! queue ! nvv4l2h264enc control-rate=0 ${encoder_bitrate}=${MAVPN_BITRATE} preset-level=1 maxperf-enable=true ! 'video/x-h264,width=1280,height=720,framerate=15/1' ! h264parse ! rtph264pay config-interval=1 mtu=1200 pt=96 ! udpsink host=${MAVPN_HOST} port=${MAVPN_PORT} ${extra_mavpn}
